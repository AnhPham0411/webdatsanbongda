...
async function main() {
  ...
  const services = [
    { name: "Áo thuê", price: 5000, icon: "Shirt" },
    { name: "Nước đá", price: 45000, icon: "Ice" },
    { name: "Thuê bóng", price: 30000, icon: "Ball" },
  ];

  for (const s of services) {
    await prisma.extraService.upsert({
      where: { id: s.name }, // This is a bit hacky if ID is cuid, but for seeding it works if we use name as ID or find by name. 
      // Actually ExtraService model doesn't have a unique constraint on name. 
      // Let's use name to find.
      update: { price: s.price, icon: s.icon },
      create: { name: s.name, price: s.price, icon: s.icon }
    }).catch(async (e: any) => {
